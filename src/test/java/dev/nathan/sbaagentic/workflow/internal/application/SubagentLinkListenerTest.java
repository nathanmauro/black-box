...
class SubagentLinkListenerTest {
    ...
    void subagentStartLinksChildToRecordedParent() {
        IngestResponse parent = ingest(PARENT_KEY, "UserPromptSubmit", "user", "Spawn a reviewer.", Map.of());
        IngestResponse child = ingest(CHILD_KEY, "SubagentStart", "agent", null, SUBAGENT_METADATA);
    ...
    void subagentStopAloneStillLinksChild() {
        IngestResponse parent = ingest(PARENT_KEY, "UserPromptSubmit", "user", "Spawn a reviewer.", Map.of());
        IngestResponse child = ingest(CHILD_KEY, "SubagentStop", "assistant", "Reviewed the diff.", SUBAGENT_METADATA);
    ...
    void missingParentSkipsLinkingSilently() {
        ...
        assertThat(jdbcTemplate.queryForObject(
                        "SELECT COUNT(*) FROM agent_sessions WHERE id = ?", Integer.class, child.sessionId()))
                .isEqualTo(1);
    }
    ...
    void nonSubagentEventsAndMissingParentMetadataAreIgnored() {
        ...
        ingest(CHILD_KEY, "SubagentStart", "agent", null, Map.of("agentId", "agent-1", "agentType", "code-reviewer"));

        assertThat(linkRows()).isEmpty();
    }

    private IngestResponse ingest(
            String clientSessionId, String eventType, String role, String text, Map<String, Object> metadata) {
