...
class EmbeddingSqlStoreTest {
    ...
    void upsertNormalizesThenReadsThroughRealSqliteFile() {
        ...
        assertThat(store.hasCurrentEmbedding("event", "event-1", "hash-1", "nomic", 2))
                .isTrue();
        assertThat(store.hasCurrentEmbedding("event", "event-1", "hash-1", "other", 2))
                .isFalse();
        assertThat(store.hasCurrentEmbedding("event", "event-1", "hash-1", "nomic", 3))
                .isFalse();
        assertThat(store.loadAll("nomic", 2)).singleElement().satisfies(stored -> {
    ...
    private static Path tempDatabase() {
        ...
        return Path.of(
                System.getProperty("java.io.tmpdir"), "bb-embedding-sql-store-test-" + UUID.randomUUID() + ".db");
    }
