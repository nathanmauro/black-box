...
class BruteForceVectorStoreTest {
    ...
    void normalizingInputsDoesNotChangeCosineRanking() {
        ...
        insertCanonical(
                normalizedFixture.jdbcTemplate(),
                "event",
                "far_aligned",
                new EmbeddingVector("nomic", new float[] {5.0f, 0.0f})
                        .normalized()
                        .values());
        insertCanonical(
                normalizedFixture.jdbcTemplate(),
                "event",
                "near_offaxis",
                new EmbeddingVector("nomic", new float[] {0.9f, 0.6f})
                        .normalized()
                        .values());
        insertCanonical(
                normalizedFixture.jdbcTemplate(),
                "event",
                "mid",
                new EmbeddingVector("nomic", new float[] {2.0f, 0.15f})
                        .normalized()
                        .values());
        ...
        List<String> rawRanking = rawFixture.vectorStore().knn(query(1.0f, 0.0f), 3, key -> true).stream()
                .map(ScoredKey::key)
                .toList();
        List<String> normalizedRanking = normalizedFixture.vectorStore().knn(query(1.0f, 0.0f), 3, key -> true).stream()
                .map(ScoredKey::key)
                .toList();
    ...
    void filterExcludesEverythingBeforeScoringResultsAreReturned() {
        ...
        assertThat(fixture.vectorStore().knn(query(1.0f, 0.0f), 3, key -> false))
                .isEmpty();
    }
    ...
    void fetchVectorsReturnsOnlyRequestedCanonicalVectorsForModelAndDimensions() {
        ...
        Map<String, EmbeddingVector> vectors =
                fixture.vectorStore().fetchVectors(List.of("event:one", "event:missing"), "nomic", 2);
    ...
    private static Fixture fixture() {
        Path database = Path.of(
                System.getProperty("java.io.tmpdir"), "bb-brute-force-vector-store-test-" + UUID.randomUUID() + ".db");
    ...
    private record Fixture(
            JdbcTemplate jdbcTemplate, EmbeddingSqlStore embeddingStore, BruteForceVectorStore vectorStore) {}
}
