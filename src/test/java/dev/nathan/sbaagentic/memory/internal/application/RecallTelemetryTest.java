...
class RecallTelemetryTest {
    ...
    private final ContextService service =
            new ContextService(events, embedder, vectors, new MemoryRecallProperties(), telemetry);
    ...
    void hybridGateSeparatesAttemptCompletionContributionAndRejectsPrivateData() throws Exception {
        ...
        when(events.recallCandidates(anyList(), any()))
                .thenReturn(List.of(
                        new RecallCandidate(accepted, "/private/project"),
                        new RecallCandidate(rejected, "/private/other")));
        when(vectors.knn(any(), anyInt(), any()))
                .thenReturn(List.of(
        ...
        try (var context = RecallRequestContext.open("mcp", "codex", "audit", "safe-project")) {
            assertThat(service.recall("private query secret", 168, null).count())
                    .isEqualTo(1);
        ...
        assertThat(record())
                ...
                .containsEntry("semantic_attempted", true)
                .containsEntry("semantic_completed", true)
                .containsEntry("semantic_contributed", true)
                .containsEntry("gate_admitted", 1)
                .containsEntry("gate_rejected", 1)
                .containsEntry("semantic_candidates", 2)
                .containsEntry("embedding_outcome", "success")
                .containsEntry("vector_outcome", "success")
                .containsEntry("result_count", 1)
                .containsEntry("no_results", false)
                .containsEntry("semantic_returned", 1)
                .containsEntry("relevance_floor", .61)
                .containsEntry("client", "codex")
                .containsEntry("purpose", "audit")
                .containsEntry("project", "safe-project");
        String json = new ObjectMapper().writeValueAsString(record());
        assertThat(json)
                .doesNotContain("private", "secret", "recalled content", "session", "fixture")
    ...
    void locationAndBlankRequestsSkipSemanticDependencies(String scope) {
        ...
        assertThat(record())
                .containsEntry("mode", "lexical")
                .containsEntry("semantic_attempted", false)
                .containsEntry("semantic_completed", false)
    ...
    void successfulSemanticSearchWithNoMatchesIsNotFallbackOrContribution() {
        ...
        assertThat(record())
                .containsEntry("mode", "hybrid")
                .containsEntry("semantic_completed", true)
                .containsEntry("semantic_contributed", false)
    ...
    void fusionContributionIsSeparateFromSemanticOverlapInTheFinalPage() {
        ...
        assertThat(service.recall("topic", 168, null, 1).items().getFirst().eventId())
                .isEqualTo("a-lexical");
        assertThat(record())
                .containsEntry("semantic_contributed", true)
                .containsEntry("semantic_hits", 1)
                .containsEntry("semantic_returned", 0)
                .containsEntry("result_count", 1);
    }
    ...
    void embeddingUnavailableFallsBackWithoutCallingQueryOrVector() {
        ...
        assertThat(record())
                ...
                .containsEntry("semantic_attempted", true)
                .containsEntry("semantic_completed", false)
                .containsEntry("fallback_reason", "embedding_unavailable")
    ...
    void embeddingErrorsPreserveLexicalResultsAndNeverExportExceptionText() {
        ...
        when(embedder.embedQuery(anyString()))
                .thenThrow(new IllegalStateException("Bearer private-secret /private/path"));
        assertThat(service.recall("topic", 168, null).count()).isEqualTo(1);
        assertThat(record())
                .containsEntry("fallback_reason", "embedding_error")
                .containsEntry("embedding_outcome", "error")
                .containsEntry("outcome", "success")
                .containsEntry("error_category", "none");
        assertThat(record().toString())
                .doesNotContain("Bearer", "private-secret", "/private/path", "IllegalStateException");
    }
    ...
    void availabilityExceptionAndCandidatesErrorAreDistinct() {
        ...
        service.recall("topic", 168, null);
        assertThat(record())
                .containsEntry("embedding_probe_outcome", "error")
                .containsEntry("fallback_reason", "embedding_error");
        ...
        service.recall("topic", 168, null);
        assertThat(record())
                .containsEntry("embedding_probe_outcome", "success")
                .containsEntry("fallback_reason", "candidates_error")
    ...
    void vectorSearchFailureFallsBackAndRetainsLexicalResults() {
        ...
        assertThat(service.recall("topic", 168, null).count()).isEqualTo(1);
        assertThat(record())
                .containsEntry("fallback_reason", "vector_error")
                .containsEntry("vector_outcome", "error")
                .containsEntry("mode", "lexical")
                .containsEntry("embedding_outcome", "success");
    }
    ...
    void optionalVectorEnrichmentFailureDoesNotDiscardRankedResults() {
        when(events.recall(anyList(), any(), any(), anyInt())).thenReturn(List.of(event("lexical", "text")));
        when(vectors.fetchVectors(any(), anyString(), anyInt()))
                .thenThrow(new IllegalStateException("secret database path"));
        var result = service.recall("topic", 168, null);
        assertThat(result.count()).isEqualTo(1);
        assertThat(result.items().getFirst().score()).isNull();
        assertThat(record())
                .containsEntry("vector_fetch_outcome", "error")
                .containsEntry("mode", "hybrid")
                .containsEntry("fallback_reason", "none")
                .containsEntry("outcome", "success");
    }
    ...
    void realRecallFailureIsRecordedAndRethrown() {
        ...
        assertThat(record())
                ...
                .containsEntry("error_category", "recall_error")
                .containsEntry("no_results", false);
    ...
    void exporterAndMetricFailuresCannotChangeRecall() {
        ...
        when(failingRegistry.counter(anyString(), any(Iterable.class)))
                .thenThrow(new IllegalStateException("metrics failure"));
        ...
        var loggingFail = new RecallTelemetry(
                registry,
                fields -> {
    ...
    void httpRequestGeneratesCorrelationAndNormalizesUntrustedAttribution() throws Exception {
        var mvc = MockMvcBuilders.standaloneSetup(
                        new ContextController(service, mock(RecordingCaptureOperations.class)))
                .build();
        var result = mvc.perform(get("/api/recall")
                        .param("scope", "/private/path")
                        .header("X-Blackbox-Client", "secret-client")
                        .header("X-Blackbox-Purpose", "Bearer secret")
                        .header("X-Blackbox-Project", "not-allowlisted")
                        .header("X-Blackbox-Recall-Id", "attacker-id"))
                .andExpect(status().isOk())
                .andReturn();
        assertThat(record())
                .containsEntry("transport", "http")
                .containsEntry("client", "unknown")
                .containsEntry("purpose", "unknown")
                .containsEntry("project", "unknown");
        assertThat(result.getResponse().getHeader("X-Blackbox-Recall-Id"))
                .isEqualTo(record().get("request_id"))
                .matches("[a-f0-9-]{36}")
                .isNotEqualTo("attacker-id");
        assertThat(RecallRequestContext.current()).isNull();
    }

    @Test
    void mcpJsonCallbackPropagatesAttributionAndRestoresContext() {
        var tools = new MemoryMcpTools(
        ...
        callback.call("""
                ...
                """);
        assertThat(record())
                .containsEntry("transport", "mcp")
                .containsEntry("client", "claude")
    ...
    void mcpInitializedClientWinsAndNestedContextRestoresEvenAfterFailure() {
        ...
        try (var outer = RecallRequestContext.open("internal", "manual", "test", null)) {
            callback.call(
                    "{\"repoOrTopic\":\"\",\"telemetryClient\":\"claude\"}",
            ...
            records.clear();
            when(events.recall(anyList(), nullable(String.class), any(), anyInt()))
                    .thenThrow(new IllegalStateException("secret"));
            assertThatThrownBy(() ->
                            callback.call("{\"repoOrTopic\":\"\"}", new ToolContext(Map.of("exchange", exchange))))
    ...
    private static AgentEvent event(String id, String text) {
        ...
        return new AgentEvent(
                ...
                null,
                null,
                Map.of("decision", text, "repo", "/private/project"),
                Instant.now());
    }
}
