...
class SbaConfigurationTest {
    ...
    void bindsExternalSummaryBackendByDefault() {
        contextRunner.run(context -> {
            assertThat(context.getEnvironment().getProperty("sba.summary.backend"))
                    .isEqualTo("external");
            assertThat(context.getBean(SummaryProperties.class).getBackend()).isEqualTo("external");
            assertThat(context.getBean(AskProperties.class).getDefaultAskCitations())
                    .isEqualTo(6);
            assertThat(context.getBean(ElasticsearchProperties.class).getIndexName())
                    .isEqualTo("sba-agentic-events");
            assertThat(context.getBean(IngestionProperties.class).getMaxTextLength())
                    .isEqualTo(20_000);
            ...
            assertThat(context.getBean(MemoryEmbeddingProperties.class).getDimensions())
                    .isEqualTo(768);
    ...
    void bindsExplicitLocalSummaryBackendOverride() {
        contextRunner.withPropertyValues("SBA_SUMMARY_BACKEND=local").run(context -> {
            assertThat(context.getEnvironment().getProperty("sba.summary.backend"))
                    .isEqualTo("local");
    ...
    void preservesEnvironmentBindingAcrossModuleOwnedProperties() {
        contextRunner
                ...
                .run(context -> {
                    ...
                    assertThat(context.getBean(MemoryEmbeddingProperties.class).isEnabled())
                            .isFalse();
                    assertThat(context.getBean(MemoryEmbeddingProperties.class).getDocumentPrefix())
                            .isEqualTo("doc:");
                    assertThat(context.getBean(MemoryEmbeddingProperties.class).getQueryPrefix())
                    ...
                    assertThat(context.getBean(IngestionProperties.class).isRedactEnabled())
                            .isFalse();
                });
    }
}
