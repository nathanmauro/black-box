...
class AuthenticationHttpTest {
    ...
    void browserLoginCaptureStreamAndLogoutRequireSessionAndCsrfWhileAgentsRemainStateless() throws Exception {
        ...
        try (var browser = HttpClient.newBuilder()
                        .cookieHandler(cookies)
                        .followRedirects(HttpClient.Redirect.NEVER)
                        .build();
                var agent = HttpClient.newBuilder()
                        .followRedirects(HttpClient.Redirect.NEVER)
                        .build()) {
            ...
            for (String path : new String[] {"/", "/index.html", "/assets/private.js", "/sessions/example"}) {
                var response = send(
                        agent,
                        get(path).header("X-Forwarded-Host", "attacker.invalid").header("X-Forwarded-Proto", "https"));
            ...
            String beforeLoginCsrf = cookies.getCookieStore().getCookies().stream()
                    .filter(cookie -> cookie.getName().equals("XSRF-TOKEN"))
                    .findFirst()
                    .orElseThrow()
                    .getValue();
            var matcher =
                    Pattern.compile("name=\"_csrf\"[^>]*value=\"([^\"]+)\"").matcher(login.body());
            ...
            assertThat(signedIn.headers().allValues("Set-Cookie").stream()
            ...
            assertThat(send(browser, postEvent("missing-csrf", null)).statusCode())
                    .isEqualTo(403);
            assertThat(send(browser, postEvent("wrong-csrf", "incorrect")).statusCode())
                    .isEqualTo(403);
            ...
            for (String malformed : new String[] {"Bearer incorrect", "Basic incorrect", "Bearer", "invalid"}) {
                assertUnauthorized(
                        send(browser, postEvent("must-not-write", csrf).header("Authorization", malformed)));
            }
            ...
            var stream = browser.send(
                    get("/api/stream").header("Accept", "text/event-stream").build(),
            ...
            try (var streamReader = new BufferedReader(new InputStreamReader(stream.body(), StandardCharsets.UTF_8))) {
                ...
                try {
                    ...
                    assertThat(closed.get(20, TimeUnit.SECONDS))
                            .as("logout closes an idle stream on its next heartbeat, without publishing an event")
                            .isTrue();
                } finally {
                    stream.body().close(); // release a blocked read before BufferedReader.close acquires its lock
                    reader.shutdownNow();
                }
                assertThat(send(agent, postEvent("after-logout-agent", null).header("Authorization", "Bearer " + TOKEN))
                                .statusCode())
                        .isEqualTo(200);
                ...
                assertThat(events.body())
                        .contains("browser-capture", "agent-capture")
                        .doesNotContain(
                                "anonymous-capture",
                                "missing-csrf-capture",
                                "wrong-csrf-capture",
                                "must-not-write-capture",
                                "after-logout-capture");
            }
        }
    }
