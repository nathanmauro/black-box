...
class CompactSearchHttpTest {
    ...
    void legacyRowLimitDoesNotBoundNestedPayload() throws Exception {
        ...
        recorder.ingest(new EventIngestRequest(
                ...
                "Read",
                null,
                Map.of("nested", nested),
                Map.of("rawHook", Map.of("nested", nested)),
                Instant.now()));
        ...
        System.out.println(
                "Synthetic legacy HTTP search bytes=" + response.body().getBytes(StandardCharsets.UTF_8).length);
        ...
        assertThat(textJson(rpc).path("items").get(0).path("eventId").asText())
                .isEqualTo(mapper.readTree(compact.body())
                        .path("items")
                        .get(0)
                        .path("eventId")
                        .asText());
    ...
    void recoveryFindsUserSourceAmongCopiesAndKeepsInferenceUnknown() throws Exception {
        ...
        String origin = seed(
                ...
                "UserPromptSubmit",
                "user",
                key + ": one authoritative backend",
                "2026-08-17T12:00:00Z");
        String proposal = seed(
                "codex",
                "proposal",
                "AssistantMessage",
                "assistant",
                key + ": engine A is a proposal only",
        ...
        for (int i = 0; i < 20; i++)
            seed(
                    ...
                    "PostToolUse",
                    "tool",
                    key + ": monitoring copy of the discussion",
        ...
        assertThat(found.path("items").get(0).path("provenance").asText())
                .isEqualTo("similar_observer_text_origin_unknown");
        ...
        JsonNode original = found.path("items").findValues("sourceReference").stream()
                .filter(item -> task.equals(item.path("externalTaskId").asText()))
                .findFirst()
                .orElseThrow();
        assertThat(original.path("externalStatus").asText()).isEqualTo("candidate_requires_verification");
        assertThat(mapper.readTree(get(original.path("eventPath").asText()).body())
                        .path("id")
                        .asText())
                .isEqualTo(origin);
        var timed = get("/api/search/compact?q=" + encode(key + " until:2026-08-17"));
        assertThat(mapper.readTree(timed.body()).path("items")).hasSize(2);
        assertThat(mapper.readTree(timed.body())
                        .path("coverage")
                        .path("elastic")
                        .path("status")
                        .asText())
                .isEqualTo("skipped_filters");
        var ungrouped = mapper.readTree(get("/api/search/compact?q=" + key + "&groupSimilar=false&limit=50")
                .body());
    ...
    void unsupportedDatesAreDiagnosedAndQuotedTokensRemainLiteral() throws Exception {
        ...
        assertThat(get("/api/search/compact?q=" + encode("since:2026-02-30")).statusCode())
                .isEqualTo(400);
        ...
        initialize();
        assertThat(textJson(call("searchContext", Map.of("query", "before:2026-08-18")))
                        .path("status")
                        .asText())
    ...
    void exactTimeBoundaryExcludesLaterFractionalSeconds() throws Exception {
        ...
        var result = mapper.readTree(get("/api/search/compact?q=" + encode(key + " until:2026-08-18T00:00:00Z"))
                .body());
        assertThat(result.path("items").findValuesAsText("eventId"))
    ...
    void exclusionRunsBeforeCandidateLimitAndExhaustionIsExplicit() throws Exception {
        ...
        for (int i = 0; i < 201; i++)
            seed("manual", "self-session", "PostToolUse", "tool", key, "2026-08-19T00:00:00Z");
        var capped = mapper.readTree(get("/api/search/compact?q=" + key).body());
        assertThat(capped.path("coverage")
                        .path("local")
                        .path("candidateLimitReached")
                        .asBoolean())
                .isTrue();
        var excluded = mapper.readTree(get("/api/search/compact?q=" + key + "&excludeSession=self-session")
                .body());
        assertThat(excluded.path("items").findValuesAsText("eventId")).containsExactly(origin);
        assertThat(excluded.path("coverage")
                        .path("local")
                        .path("candidateLimitReached")
                        .asBoolean())
                .isFalse();
