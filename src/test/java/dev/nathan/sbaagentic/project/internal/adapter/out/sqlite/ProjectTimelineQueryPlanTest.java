...
class ProjectTimelineQueryPlanTest {
    ...
    void seedTimelineProjects() {
        ...
        insertEvent("typed-decision", ROOT_SESSION, "dEcIsIoN", null, null, null, null,
                "2026-08-05T10:00:00Z");
        insertEvent("metadata-decision", ROOT_SESSION, "UserPromptSubmit", "user", null, null,
                "{\"KiNd\":\"DeCiSiOn\"}", "2026-08-05T10:01:00Z");
        insertEvent("assistant-text", ROOT_SESSION, "AssistantMessage", "AsSiStAnT",
                "Implemented the indexed timeline query", null, null, "2026-08-05T10:02:00Z");
        insertEvent("assistant-whitespace", ROOT_SESSION, "Unknown", "assistant", "   ", null, null,
                "2026-08-05T10:03:00Z");
        insertEvent("assistant-empty", ROOT_SESSION, "AssistantMessage", "assistant", "", null,
                null, "2026-08-05T10:04:00Z");
        insertEvent("assistant-null-text", ROOT_SESSION, "AssistantMessage", "assistant", null,
                null, null, "2026-08-05T10:05:00Z");
        insertEvent("large-tool-metadata", ROOT_SESSION, "PostUse", "user", null, "sqlite3",
                largeMetadata(), "2026-08-05T10:06:00Z");
        insertEvent("large-metadata-noise", ROOT_SESSION, "UserPromptSubmit", "user", null, null,
                largeMetadata(), "2026-08-05T10:06:30Z");
        insertEvent("event-type-tool", ROOT_SESSION, "PreToOlUse", null, null, null, null,
                "2026-08-05T10:07:00Z");
        insertEvent("event-type-error", ROOT_SESSION, "RuntimeErRoRReported", "user", null, null,
                null, "2026-08-05T10:08:00Z");
        insertEvent("event-type-fail", ROOT_SESSION, "HardFaIlure", "user", null, null, null,
                "2026-08-05T10:09:00Z");
        insertEvent("metadata-handoff", ROOT_SESSION, "UserPromptSubmit", "user", null, null,
                "{\"KIND\":\"HaNdOfF\"}", "2026-08-05T10:10:00Z");
        insertEvent("metadata-guard-miss-assistant", ROOT_SESSION, "AssistantMessage", "assistant",
                "The metadata guard must stay last", null, "{\"kind\":\"noise\"}",
                "2026-08-05T10:11:00Z");
        insertEvent("metadata-guard-miss-noise", ROOT_SESSION, "UserPromptSubmit", "user", null,
                null, "{\"kind\":\"noise\"}", "2026-08-05T10:12:00Z");
        insertEvent("null-columns-noise", ROOT_SESSION, "Unknown", null, null, null, null,
                "2026-08-05T10:13:00Z");

        insertEvent("alias-one-typed-handoff", ALIAS_ONE_SESSION, "hAnDoFf", null, null, null,
                null, "2026-08-05T10:14:00Z");
        insertEvent("alias-two-metadata-handoff", ALIAS_TWO_SESSION, "UserPromptSubmit", "user",
                null, null, "{\"Kind\":\"HANDOFF\"}", "2026-08-05T10:15:00Z");
        insertEvent("other-typed-decision", OTHER_SESSION, "DeCiSiOn", null, null, null, null,
    ...
    void shortCircuitedStorylinePredicateSelectsExactlyTheOldPredicateRows() {
        ...
        assertThat(shortCircuitedIds).containsExactly(
                ...
                "typed-decision");
        assertThat(shortCircuitedIds).doesNotContain(
        ...
        assertThat(ProjectRepository.STORYLINE_PREDICATE).containsSubsequence(
        ...
        assertThat(ProjectRepository.STORYLINE_PREDICATE.lastIndexOf("WHEN "))
                .isEqualTo(ProjectRepository.STORYLINE_PREDICATE.indexOf(
                        "WHEN e.metadata_json LIKE '%\"kind\":\"%'"));
    }
    ...
    void timelineQueriesKeepOldResultsCountsOrderingPaginationJoinsAliasScopesAndIndexPlans() {
        ...
        assertThat(fullIds).containsExactly(
                "typed-decision",
                "metadata-decision",
                "assistant-text",
                "large-tool-metadata",
                ...
                "alias-two-meld",
                "alias-two-metadata-handoff");
        assertThat(fullIds).doesNotContain(
                "assistant-whitespace",
                "large-metadata-noise",
                "metadata-guard-miss-noise",
                "other-typed-decision");
        assertThat(block(fullTimeline, "typed-decision").sessionTitle())
                .isEqualTo("Root timeline session");
        ...
        assertThat(rootSessionIds).containsExactly(
                "typed-decision",
                "metadata-decision",
                "assistant-text",
                "large-tool-metadata",
        ...
        assertThat(queryIds(oldTimelineSql,
                ROOT, ALIAS_ONE, ALIAS_TWO, ROOT, ALIAS_ONE, ALIAS_TWO, 50, 0))
                .containsExactlyElementsOf(fullIds);

        String oldSessionSql = withOldStorylinePredicate(
                ProjectRepository.timelineBlocksForSessionSql(3));
        ...
        assertUsesSessionEventIndex(explain(
                ProjectRepository.countTimelineBlocksSql(3),
                ROOT, ALIAS_ONE, ALIAS_TWO, ROOT, ALIAS_ONE, ALIAS_TWO));
        assertUsesSessionEventIndex(explain(
                ProjectRepository.timelineBlocksSql(3),
                ROOT, ALIAS_ONE, ALIAS_TWO, ROOT, ALIAS_ONE, ALIAS_TWO, 50, 0));
        assertUsesSessionEventIndex(explain(
                ProjectRepository.timelineBlocksForSessionSql(3),
                ROOT, ALIAS_ONE, ALIAS_TWO, ROOT_SESSION, 50));
    }
    ...
    private void insertSession(String sessionId, String title, String cwd, int eventCount) {
        jdbcTemplate.update("""
                ...
                "2026-08-05T10:00:00Z", "2026-08-05T10:20:00Z", eventCount);
    }
    ...
    private void insertEvent(
        ...
        jdbcTemplate.update("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                eventId, sessionId, "codex", sessionId, eventType, role, text,
    ...
    private void insertMeld(String id, String projectKey, String title, String createdAt) {
        jdbcTemplate.update("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                id, projectKey, title, "Saved timeline synthesis", "local", "context-bundle",
                "project-meld-v1", "export_bundle", 1, null, createdAt);
    }
    ...
    private void assertPredicateEquivalent(
            ...
            String toolName,
            String metadataJson) {
        Integer oldResult = predicateResult(
                OLD_STORYLINE_PREDICATE, eventType, role, eventText, toolName, metadataJson);
        Integer shortCircuitedResult = predicateResult(
                ProjectRepository.STORYLINE_PREDICATE,
                eventType,
                role,
                eventText,
                toolName,
        ...
        return jdbcTemplate.queryForObject("""
                SELECT %s AS matched
                  FROM (SELECT ? AS event_type,
                               ...
                               ? AS tool_name,
                               ? AS metadata_json) e
                """.formatted(predicate), Integer.class,
                eventType, role, eventText, toolName, metadataJson);
    }
