...
class JudgmentStandaloneIntegrationTest {
    ...
    @Autowired FailingTransport transport;
    ...
    void captureAndRecallWorkWhileJudgeIsBlockedAndAfterItFailsWithoutAnyConsumer() throws Exception {
        ...
        try {
            ...
            assertThat(http.getForObject("/api/events/" + id, JsonNode.class).path("text").asText())
                    .isEqualTo(text);
            assertThat(http.getForObject("/api/recall?scope=/fixtures/standalone-cortex", JsonNode.class)
                    .path("items").toString()).contains(id);
        } finally {
            transport.release.countDown();
        }
        await().atMost(Duration.ofSeconds(5)).untilAsserted(() ->
                assertThat(http.getForObject("/api/health/judge", JsonNode.class).path("failures").asLong())
                        .isPositive());
        ...
        assertThat(http.getForObject("/api/recall?scope=/fixtures/standalone-cortex", JsonNode.class)
                .path("items").toString()).contains(afterFailure.getBody().eventId());
    }

    private static EventIngestRequest request(String turn, String text) {
        return new EventIngestRequest("codex", "standalone-cortex", turn, "Decision", "assistant", text,
                "/fixtures/standalone-cortex", null, null, null, Map.of(), Instant.now());
    }
    ...
    static class FakeProvider {
        @Bean @Primary
        FailingTransport failingTransport() { return new FailingTransport(); }
    }
}
