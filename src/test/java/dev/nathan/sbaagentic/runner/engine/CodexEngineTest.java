...
class CodexEngineTest {
    ...
    void workspaceWriteSandboxAllowsLoopbackReporting() {
        List<String> command = engine.command(
                "prompt", new EngineConfig("codex", "gpt-5.6-sol", "ultra", "workspace-write", null, true), null);

        assertThat(command)
                .containsSequence("--sandbox", "workspace-write", "-c", "sandbox_workspace_write.network_access=true");
    }

    @Test
    void otherSandboxModesGetNoNetworkOverride() {
        List<String> command = engine.command(
                "prompt", new EngineConfig("codex", "gpt-5.6-sol", "ultra", "read-only", null, true), null);
    ...
    void linkedWorktreeGitCommonDirBecomesWritableRoot(@TempDir File repo) throws IOException {
        ...
        Files.writeString(new File(worktree, ".git").toPath(), "gitdir: " + gitDir.getAbsolutePath() + "\n");

        List<String> command = engine.command(
                "prompt", new EngineConfig("codex", "gpt-5.6-sol", "ultra", "workspace-write", null, true), worktree);

        assertThat(command)
                .containsSequence(
                        "-c",
                        "sandbox_workspace_write.writable_roots=[\"" + new File(repo, ".git").getAbsolutePath()
                                + "\"]");
    }
    ...
    void regularCheckoutGetsNoWritableRootOverride(@TempDir File repo) {
        List<String> command = engine.command(
                "prompt", new EngineConfig("codex", "gpt-5.6-sol", "ultra", "workspace-write", null, true), repo);
    ...
    void worktreeDirIsPreTrustedBeforeEndOfOptionsMarker(@TempDir File worktree) throws IOException {
        ...
        assertThat(override).contains("\"" + worktree.getCanonicalPath() + "\"={trust_level=\"trusted\"}");
    ...
    void symlinkedWorktreeEmitsBothRawAndPhysicalTrustEntries(@TempDir File tempDir) throws IOException {
        ...
        String override = projectsOverride(engine.command(
                "prompt", new EngineConfig("codex", "gpt-5.6-sol", "ultra", "workspace-write", null, true), link));
    ...
    void quotesInWorktreePathAreTomlEscaped(@TempDir File tempDir) {
        ...
        String override = projectsOverride(engine.command(
                "prompt", new EngineConfig("codex", "gpt-5.6-sol", "ultra", "workspace-write", null, true), weird));
    ...
    void nullWorktreeGetsNoTrustOverride() {
        List<String> command =
                engine.command("prompt", new EngineConfig("codex", "gpt-5.6-sol", "ultra", null, null, true), null);
    ...
    void dashLeadingPromptIsNeverParsedAsFlag() {
        List<String> command = engine.command(
                "--shout looks like a flag", new EngineConfig("codex", "gpt-5.6-sol", "xhigh", null, null, true), null);
