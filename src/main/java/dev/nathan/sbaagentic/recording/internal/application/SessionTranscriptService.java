...
public class SessionTranscriptService implements SessionTranscriptOperations {
    ...
    private static final Comparator<AgentEvent> NEWEST_FIRST = Comparator
            .comparing(AgentEvent::observedAt)
    ...
    public SessionTranscriptResponse transcript(String sessionId, String query, String before, int limit) {
        ...
        List<AgentEvent> recordedConversation = transcript.messages().isEmpty()
                ? List.of()
                : repository.conversationEventsForSession(sessionId);
        ...
        List<AgentEvent> page = combined.size() > safeLimit
                ? List.copyOf(combined.subList(0, safeLimit))
    ...
    private boolean matches(AgentEvent event, AgentSession session, EventQuery query) {
        ...
        if (!included(source, query.values(Field.SOURCE))) return false;
        if (excluded(source, query.excluded(Field.SOURCE))) return false;
        if (!matchesTranscriptKind(event, query.values(Field.KIND))) return false;
        if (excluded(kind, query.excluded(Field.KIND))) return false;
        if (!query.values(Field.TOOL).isEmpty()) return false;
        if (!includedBySubstring(cwd, query.values(Field.PROJECT))) return false;
        if (excludedBySubstring(cwd, query.excluded(Field.PROJECT))) return false;
        if (!query.values(Field.PROJECT_EXACT).isEmpty()
                && query.values(Field.PROJECT_EXACT).stream().map(SessionTranscriptService::lower)
                        .noneMatch(cwd::equals)) return false;
        if (query.excluded(Field.PROJECT_EXACT).stream().map(SessionTranscriptService::lower)
                .anyMatch(cwd::equals)) return false;
        String haystack = lower(event.text());
        if (query.freeTerms().stream().map(SessionTranscriptService::lower)
                .anyMatch(term -> !haystack.contains(term))) return false;
        if (query.sinceSpec().isPresent()
                && event.observedAt().isBefore(query.sinceSpec().orElseThrow().resolve(clock))) return false;
    ...
    private static boolean included(String value, List<String> included) {
        return included.isEmpty() || included.stream().map(SessionTranscriptService::lower).anyMatch(value::equals);
    }
    ...
    private static boolean includedBySubstring(String value, List<String> included) {
        return included.isEmpty() || included.stream().map(SessionTranscriptService::lower).anyMatch(value::contains);
    }
    ...
    private static boolean duplicates(AgentEvent recorded, AgentEvent transcript) {
        ...
        if (recordedRole == null || !recordedRole.equals(transcript.role())) return false;
        if (!normalize(recorded.text()).equals(normalize(transcript.text()))) return false;
        if (recorded.turnId() != null && transcript.turnId() != null
                && recorded.turnId().equals(transcript.turnId())) return true;
        return Duration.between(recorded.observedAt(), transcript.observedAt()).abs()
                .compareTo(DUPLICATE_WINDOW) <= 0;
    }

    private static String conversationRole(AgentEvent event) {
        String role = lower(event.role());
        if ("user".equals(role) || "assistant".equals(role)) return role;
        String type = lower(event.eventType()).replaceAll("[^a-z0-9]", "");
        if (type.equals("userpromptsubmit") || type.equals("beforesubmitprompt")) return "user";
        if (event.text() != null && !event.text().isBlank()
                && (type.equals("stop") || type.equals("assistantmessage") || type.equals("agentmessage")
                        || type.equals("agentresponse") || type.equals("finalresponse"))) return "assistant";
        return null;
    }
