...
public class CompactSearchMcpTools implements Supplier<ToolCallback[]> {
    ...
    public CompactSearchMcpTools(CompactSearchOperations search) {
        this.search = search;
    }

    @Tool(
            description =
                    "Discover captured evidence with bounded excerpts and source references. Prefer this over raw searchSessions. "
                            ...
                            + "Source candidates and matching text do not prove origin or causation.",
            resultConverter = JsonConverter.class)
    public CompactSearchResult searchContext(
            @ToolParam(
                            description =
                                    "Nonblank query, at most 1024 UTF-16 units. Quote a whole operator token to search it literally.")
                    String query,
            @ToolParam(required = false, description = "Global result limit; default 10, maximum 50.") Integer limit,
            @ToolParam(
                            required = false,
                            description =
                                    "Serialized application JSON UTF-8 byte budget; default 24000, range 2048–64000. MCP framing is additional.")
                    Integer maxBytes,
            @ToolParam(
                            required = false,
                            description =
                                    "Exclude one exact internal or client session ID before selecting candidates.")
                    String excludeSession,
            @ToolParam(
                            required = false,
                            description =
                                    "Default true: group identical complete observer text as origin-unknown similarity, never decisions. False lists individual events.")
                    Boolean groupSimilar) {
    ...
    public static final class JsonConverter implements ToolCallResultConverter {
        ...
        public String convert(Object result, Type returnType) {
            ...
            return CompactSearchJson.write(result);
        }
    }
}
