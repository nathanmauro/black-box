    ...
    public static List<String> validate(String query) {
        ...
        for (String token : EventQuery.tokenize(query)) {
            ...
            if (name.equals("before")) {
                ...
                return List.of(
                        "Unsupported before: operator. Use until:YYYY-MM-DD to include that entire day "
            ...
            if (List.of("since", "until", "last").contains(name)) {
                TimeSpec spec = name.equals("last")
                        ? TimeSpec.parseDurationOnly(value, TimeSpec.Edge.START)
                        : TimeSpec.parse(value, name.equals("until") ? TimeSpec.Edge.END : TimeSpec.Edge.START);
                if (spec == null || negated) {

                    return List.of(
                            "Invalid time filter. Use since:/until: with an ISO date or timestamp, "
