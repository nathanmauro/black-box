...
public class AskService implements AskOperations {
    ...
    public AskResponse ask(AskRequest request) {
        ...
        if (citations.isEmpty()) {
            ...
            return new AskResponse(
                    question, NO_HIT_ANSWER, retrieval.mode(), "unavailable".equals(retrieval.mode()), citations);
        }
        try {
            ...
            String answer = "Local answer synthesis is unavailable, but retrieval found " + citations.size()
                    + " memory source(s). Review the citations below.";
    ...
    private Retrieval retrieveHits(String query, int limit) {
        ...
        try {
            ...
            return new Retrieval(
                    "hybrid", false, "hybrid bm25 + vector", ReciprocalRankFusion.fuse(lexical, vector, limit));
        } catch (RuntimeException ex) {

            return new Retrieval(
                    "bm25", true, ex.getMessage(), lexical.stream().limit(limit).toList());
        }
    }
    ...
    private List<AskCitation> citations(List<MemoryHit> hits) {
        ...
        return hits.stream().map(hit -> citation(hit, number.getAndIncrement())).toList();
    }
