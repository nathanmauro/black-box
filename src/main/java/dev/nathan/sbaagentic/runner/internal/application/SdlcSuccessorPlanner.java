...
public class SdlcSuccessorPlanner {
    ...
    public void reconcilePlan(Task task, TaskSpec spec, List<TaskEvent> events, String actorId) {
        if (state.existingBuildTask(spec.id()).isPresent()
                || approvalInterpreter.hasRunnerMarker(events, actorId, ApprovalInterpreter.REJECTION_RECORDED)) {
