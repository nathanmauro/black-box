    ...
    public void execute(
        ...
        try {
            Optional<StageContext> loaded =
                    contextLoader.loadSdlcContext(claimedReviewTask, config, actorId, "SDLC review");
            if (loaded.isEmpty()) {
            ...
            if (!stateReader.hasApprovedPlan(context.spec().id(), actorId)) {
                block(task.id(), actorId, "SDLC review requires an approved, un-rejected plan for its spec.");

                return;
            }
            Optional<Task> planTask = stateReader.latestTask(context.spec().id(), "sdlc:plan", TaskStatus.DONE);
            Optional<String> approvedPlan =
                    planTask.flatMap(plan -> stateReader.latestWorkerAnnotationText(plan.id(), "plan", null));
            ...
            String prompt = goalPromptBuilder.buildReview(
                    task.id(), context.spec().body(), context.resolvedVerify(), approvedPlan.orElseThrow());
            ...
            switch (result.outcome()) {
                ...
                case DONE -> {
                    Optional<String> review =
                            stateReader.latestWorkerAnnotationText(task.id(), "review", result.startedAt());
                    if (review.isEmpty()) {
                        block(task.id(), actorId, "SDLC review worker reported done without a review annotation.");
            ...
            releaseToOpenBestEffort(task.id(), actorId, "SDLC review execution crashed: " + ex.getMessage());
        } finally {
