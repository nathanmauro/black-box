        ...
        try {
            ...
            switch (result.outcome()) {
                ...
                case TIMED_OUT, BLOCKED -> {
                    ...
                    block(task.id(), actorId, result.outcome() == WorkerRunExecutor.WorkerOutcome.TIMED_OUT
                            ? "Run timed out after 45m. " + result.detail() : result.detail());
                }
                case DONE -> {
                    ...
                    if (plan.isEmpty()) {
                        worktreeManager.reportRecovery(task.id(), actorId, orchestratorSessionId,
                                worktreeDir, branchName, checkpoint, "Plan annotation missing; worktree retained for inspection.");
                    ...
                    if (!before.equals(worktreeManager.gitState(worktreeDir))) {
                        worktreeManager.reportRecovery(task.id(), actorId, orchestratorSessionId,
                                worktreeDir, branchName, checkpoint, "Plan worker changed the worktree; output retained for inspection.");
                        block(task.id(), actorId,
            ...
            String recovery = worktreeManager.reportRecovery(task.id(), actorId, orchestratorSessionId,
                    worktreeDir, branchName, checkpoint, disposition);
            releaseToOpenBestEffort(task.id(), actorId, "SDLC plan execution crashed: " + ex.getMessage() + ". " + recovery);
        }
        finally {
            ...
            if (completed) {
                String disposition = cleanup(createdWorktree, tmuxSessionName);
                worktreeManager.reportRecovery(task.id(), actorId, orchestratorSessionId,
                        worktreeDir, branchName, "plan completion acknowledged", disposition);
            }
        }
    }
