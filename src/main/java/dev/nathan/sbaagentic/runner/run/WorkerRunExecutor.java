...
public final class WorkerRunExecutor {
    ...
    public WorkerRunResult execute(
        ...
        if (selected.isEmpty()) {
            ...
            return new WorkerRunResult(WorkerOutcome.NO_ENGINE, "No enabled engine configured", tmuxSessionName, null);
        }
        ...
        apiClient.annotate(
                ...
                actorId,
                "progress",
                "Engine '" + selection.engine().id() + "' launched in tmux session " + tmuxSessionName + ".",
                null);

        CompletionResult result = null;
        while (Instant.now().isBefore(runDeadline)) {
            Duration remaining = Duration.between(Instant.now(), runDeadline);
            Duration slice = remaining.compareTo(COMPLETION_SLICE_TIMEOUT) < 0 ? remaining : COMPLETION_SLICE_TIMEOUT;
            if (slice.isZero() || slice.isNegative()) {
                break;
            }
            Duration pollInterval = slice.compareTo(COMPLETION_POLL_INTERVAL) < 0 ? slice : COMPLETION_POLL_INTERVAL;
            result = completionDetector.awaitCompletion(
                    task.id(), tmuxSessionName, worktreeDir, slice, pollInterval, runStart);
            if (result.outcome() != CompletionDetector.Outcome.TIMED_OUT) {
                break;
            }

            if (Duration.between(lastIncrementalIngestAt, Instant.now()).compareTo(Duration.ofSeconds(60)) >= 0) {
                try {
                    workerSessionIngest.ingestIncremental(worktreeDir, task.id(), actorId, orchestratorSessionId);
            ...
            notifyBestEffort(
                    config,
                    repoDir,
                    "Black Box task " + task.id() + " engine " + limitedEngineId + " was rate-limited.");
            ...
            if (fallback.isEmpty()) {
                ...
                notifyBestEffort(
                        config,
                        repoDir,
                        "Black Box task " + task.id() + " is open again because no fallback engine is configured.");

                return new WorkerRunResult(WorkerOutcome.REQUEUED, reason, tmuxSessionName, runStart);
            }
            ...
            apiClient.annotate(
                    ...
                    actorId,
                    "progress",
                    "Relaunched with fallback engine '" + selection.engine().id() + "' in tmux session "
                            + tmuxSessionName + ".",
                    null);
        }
        if (result == null
                || (result.outcome() == CompletionDetector.Outcome.TIMED_OUT
                        && !Instant.now().isBefore(runDeadline))) {
            String detail =
                    result == null ? "Run deadline elapsed before a completion result was available." : result.detail();
            result = new CompletionResult(CompletionDetector.Outcome.TIMED_OUT, detail);
        }
        try {
            workerSessionIngest.ingestAndLink(worktreeDir, task.id(), actorId, orchestratorSessionId);
        } catch (RuntimeException ex) {
    ...
    private void launchEngine(
        ...
        if (command == null || command.isEmpty()) {
            throw new IllegalStateException("Engine '" + selection.engine().id() + "' returned no command");
        }
        ...
        if ("fake".equals(selection.engine().id())) {
            commandLine += "; export SBA_TASK_ID=" + shQuote(taskId) + "; export SBA_WORKTREE="
                    + shQuote(worktreeDir.getAbsolutePath());
        }
    ...
    private Optional<SelectedEngine> nextEngine(RunnerConfig config, SelectedEngine currentSelection) {
        List<EngineConfig> configuredEngines = safeList(config.engines());
        for (int index = currentSelection.configIndex() + 1; index < configuredEngines.size(); index++) {
            Optional<SelectedEngine> selected = selectedEngine(configuredEngines, index);
            if (selected.isPresent()
                    && !"fake".equals(selected.orElseThrow().engine().id())) {
    ...
    private Optional<SelectedEngine> selectedEngine(List<EngineConfig> configuredEngines, int index) {
    ...
    private void notifyBestEffort(RunnerConfig config, File workingDir, String message) {
        ...
        try {
            ProcessResult result = processRunner.run(List.of("/bin/sh", "-c", command), workingDir, NOTIFY_TIMEOUT);
    ...
    public record WorkerRunResult(WorkerOutcome outcome, String detail, String tmuxSessionName, Instant startedAt) {}
