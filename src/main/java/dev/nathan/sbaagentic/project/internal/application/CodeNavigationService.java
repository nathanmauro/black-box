...
public class CodeNavigationService implements CodeNavigationOperations {
    ...
    public CodeNavigationService(CodeScopeResolver resolver, CommandLauncher launcher, EditorProperties properties) {
    ...
    public CodeNavigationResult openInEditor(CodeReference reference) {
        ...
        try {
            launcher.launch(List.of(editor.toString(), resolved.workspaceRoot().toString()), properties.getTimeout());
            launcher.launch(List.of(editor.toString(), "-g", gotoTarget(resolved)), properties.getTimeout());
        } catch (CommandLaunchException ex) {
            throw failure(CodeNavigationError.EDITOR_DISABLED, "The configured editor could not be opened.", ex);
        }
    ...
    public CodeNavigationResult revealInFinder(CodeReference reference) {
        ...
        if (!Files.isRegularFile(command) || !Files.isExecutable(command)) {
            throw failure(CodeNavigationError.REVEAL_UNAVAILABLE, "Finder reveal is unavailable on this machine.");
        }
        try {
            launcher.launch(List.of(FINDER_COMMAND, "-R", resolved.target().toString()), properties.getTimeout());
        } catch (CommandLaunchException ex) {
            throw failure(CodeNavigationError.REVEAL_UNAVAILABLE, "Finder could not reveal the file.", ex);
        }
    ...
    private Path verifiedEditorCommand() {
        ...
        try {
            ...
            throw failure(CodeNavigationError.EDITOR_DISABLED, "Open in editor is disabled or misconfigured.", ex);
        }
    }
    ...
    private static CodeNavigationException editorDisabled() {
        ...
        return failure(CodeNavigationError.EDITOR_DISABLED, "Open in editor is disabled or misconfigured.");
    }
