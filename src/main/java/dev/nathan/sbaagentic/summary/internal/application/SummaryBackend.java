...
public class SummaryBackend implements ProjectMeldSummarizer {
    ...
    public SummaryBackend(
            SummaryProperties properties, LocalSummaryModel localAiClient, ExternalSummaryModel externalSummaryClient) {
    ...
    public String summarize(String transcript) {
        if ("external".equalsIgnoreCase(properties.getBackend())) {
            ...
            return externalSummaryClient
                    .summarize(transcript)
                    .orElseThrow(
                            () -> new IllegalStateException("External summary command failed or produced no summary"));
        }
