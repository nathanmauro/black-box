...
branch_lineage:
  - round: 1
    ...
    commit: "d91fed5"
    status: "rework"
    note: "Review found the adopted-done path completes a task without shipping it, adoption ignores task lane (SDLC build never chains sdlc:review; SDLC plan skips its validation gates), and sdlc:review resolves a worktree path that never exists because a review runs inside the build task's worktree. Round 2 was rebased off this branch onto main and stands alone; round 1 needs a lane-scoped redesign with a ship-already-ran marker before it can land."
---
