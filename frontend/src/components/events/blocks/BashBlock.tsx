    ...
    <div class="bash-block">
      ...
      <div class="bash-meta">
        <Show when={props.block.cwd}>
          {(cwd) => (
            <span class="bash-cwd" title={cwd()}>
              {truncatePath(cwd())}
            </span>
          )}
        </Show>
        <Show when={props.block.exitCode !== null}>
          <span classList={{ "bash-exit": true, "bash-exit--error": failed() }}>
            exit {props.block.exitCode}
          </span>
        </Show>
      ...
      <Show when={props.block.output}>
        {(output) => (
          <LazyDetails
            summary={`Output (${output().length.toLocaleString("en-US")} chars)`}
            class="detail-block detail-block--output"
          >
