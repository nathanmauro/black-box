...
export function sessionConversationRole(event: AgentEvent): SessionConversationRole | null {
  if (isSessionMemoryEvent(event) || isSessionToolEvent(event) || isHiddenSessionEvent(event)) return null;
...
export function mergeSessionEvents(recorded: AgentEvent[], transcript: AgentEvent[]): AgentEvent[] {
  ...
  const add = (event: AgentEvent, origin: Candidate["origin"], index: number) => {
    ...
    const duplicate = candidate.identities.some((identity) =>
      (byIdentity.get(identity) ?? []).some((existing) => clearlySameOccurrence(existing.event, event)),
    );
    if (duplicate) return;
...
export function filterSessionTranscriptTurns<T extends SessionTranscriptTurnLike>(turns: T[], query: string): T[] {
  const terms = sessionSearchTerms(query);
  if (!terms.length) return turns;
  return turns.filter((turn) => {
    const fields = turn.events.flatMap(searchableEventFields).map(normalizedSearchText).filter(Boolean);
...
function semanticIdentities(event: AgentEvent): string[] {
  ...
  if (event.toolInputJson?.trim()) identities.push(`tool:${tool}:input:${payloadIdentity(event.toolInputJson)}`);
  if (event.toolOutputJson?.trim()) identities.push(`tool:${tool}:output:${payloadIdentity(event.toolOutputJson)}`);
  return identities;
}
