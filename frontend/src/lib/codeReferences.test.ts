...
describe("toCodeReference", () => {
  ...
  it("uses a boundary-aware longest-prefix match and preserves the exact worktree key", () => {
    ...
    expect(
      toCodeReference({ path: "/Users/nathan/Developer/proj/example-other/src/App.ts" }, scopes),
    ).toBeNull();
  });
  ...
  it("fails closed for broad roots, relative paths, traversal, and unmatched paths", () => {
    expect(
      toCodeReference({ path: "/etc/passwd" }, [{ projectKey: "root", root: "/" }]),
    ).toBeNull();
    expect(
      toCodeReference({ path: "/Users/nathan/.ssh/config" }, [
        { projectKey: "home", root: "/Users/nathan" },
      ]),
    ).toBeNull();
    expect(toCodeReference({ path: "src/App.ts" }, scopes)).toBeNull();
    expect(
      toCodeReference({ path: "/Users/nathan/Developer/proj/example/src/../secret" }, scopes),
    ).toBeNull();
