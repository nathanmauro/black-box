...
export function bashPresenter(event: AgentEvent): Presentation {
  ...
  const failed =
    (result.exitCode !== null && result.exitCode !== 0) || outputLooksFailed(event.toolOutputJson);
  ...
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    const exitCode =
      typeof exitRaw === "number"
        ? exitRaw
        : typeof exitRaw === "string" && exitRaw.trim() !== "" && Number.isFinite(Number(exitRaw))
          ? Number(exitRaw)
          : null;
    const wallRaw = record.wall_time ?? record.wall_time_seconds ?? record.wallTime;
    const wallTime =
      typeof wallRaw === "string" ? wallRaw : typeof wallRaw === "number" ? String(wallRaw) : null;
