...
describe("modeMessage", () => {
  ...
  it("grows the mini width to fit a wider measured chip, so a long pulse label and a two-digit count are never clipped", () => {
    expect(modeMessage("mini", 168)).toEqual({ type: "mode", mode: "mini", width: 168, height: 36 });
  });
  ...
  it("ignores a non-finite or non-positive measurement", () => {
    expect(modeMessage("mini", Number.NaN)).toEqual({ type: "mode", mode: "mini", width: 132, height: 36 });
    expect(modeMessage("mini", Number.POSITIVE_INFINITY)).toEqual({ type: "mode", mode: "mini", width: 132, height: 36 });
    expect(modeMessage("mini", -10)).toEqual({ type: "mode", mode: "mini", width: 132, height: 36 });
    expect(modeMessage("mini", 0)).toEqual({ type: "mode", mode: "mini", width: 132, height: 36 });
  });

  it("ignores the measurement outside mini mode", () => {
    expect(modeMessage("compact", 999)).toEqual({ type: "mode", mode: "compact", width: 340, height: 420 });
  });
});
