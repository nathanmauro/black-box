...
const server = spawn('java', ['-jar', 'target/sba-agentic-0.2.0.jar'], {
  ...
  env: { ...process.env, SBA_PORT: String(port), SBA_BIND_ADDRESS: '127.0.0.1',
    ...
    SBA_AUTH_ENABLED: 'true', SBA_AUTH_SECURE_COOKIES: 'false', SBA_AUTH_USERNAME: 'blackbox',
    SBA_AUTH_PASSWORD: password, SBA_AUTH_API_TOKEN: apiToken,
    SBA_LOCAL_AI_ENABLED: 'false', SBA_ELASTICSEARCH_ENABLED: 'false', SBA_SUMMARY_BACKEND: 'local',
    SBA_MEMORY_EMBEDDING_ENABLED: 'false', SBA_EDITOR_ENABLED: 'false' },
});
...
try {
  ...
  await page.getByLabel('Goal', { exact: true }).fill('Prove authenticated frontend mutations use current CSRF credentials.');
  const specResponse = page.waitForResponse(response => response.url().endsWith('/api/specs') && response.request().method() === 'POST');
  const taskResponse = page.waitForResponse(response => response.url().endsWith('/api/tasks') && response.request().method() === 'POST');
  ...
  await Promise.all([page.waitForURL(url => url.pathname === '/login'), page.getByRole('button', { name: 'Log Out' }).click()]);
  assert.equal(await page.evaluate(async () => (await fetch('/api/status')).status), 401);
  console.log(JSON.stringify({ passed: true, flow: 'login -> read -> actual UI create spec/task -> logout -> anonymous denial', fixture }));
} finally {
  await browser?.close();
  server.kill('SIGTERM');
